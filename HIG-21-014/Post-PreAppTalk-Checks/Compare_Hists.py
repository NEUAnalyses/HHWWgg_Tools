    ...
    if(compareFullandCategorized):
        for node in nodes:
            ...
            #         # exec("ax_%s = axarr_node[%s]"%(i_year))
                    ...
                    plt.savefig("{ol}/{node}_{year}_{cat}.pdf".format(ol=ol, node=node, year=year, cat=cat))
                    plt.savefig("{ol}/{node}_{year}_{cat}.png".format(ol=ol, node=node, year=year, cat=cat))
                    plt.close()
            ...
            for i_year, year in enumerate(years):
                ...
                if(EFT_DNN):
                    ...
                    if(variable == "evalDNN_HH"): 
                        EFT_variable = "evalDNN"
                    Node_variable = Node_tree[EFT_variable].array()
                ...
                plt.savefig("{ol}/{node}_{year}.png".format(ol=ol, node=node, year=year))
                plt.close()
    ...
    if(plotReweighted):
        for node in nodes:
            ...
            for year_i, year in enumerate(years):
                ...
                ol = "/eos/user/a/atishelm/www/HHWWgg/HIG-21-014/PostPreAppTalkChecks/SemileptonicEFTReweighting/"
                fig.savefig("{ol}/{node}.pdf".format(ol=ol, node=node))
                fig.savefig("{ol}/{node}.png".format(ol=ol, node=node))
                plt.close()
