...
std::pair<double,double> METXYCorr_Met_MetPhi(double uncormet, double uncormet_phi, int runnb, int year, bool isMC, int npv){
  // cout << uncormet << " " << uncormet_phi << " " << runnb << " " << year << " " << isMC << " " << npv << endl;
  ...
  if(isMC && year == 2016) runera = y2016MC;
    ...
    cout << "METXYCorr_Met_MetPhi(): Couldn't find data/MC era => no correction applied" << endl;
  ...
  // cout << uncormet << " " << METxcorr << " " << METycorr << endl;
